...
class InstallerFeatureTests(unittest.TestCase):
    ...
    def test_final_summary_masks_superadmin_password(self):
        self.assertNotIn('echo "Password superadmin (database): $OE_SUPERADMIN"', UBUNTU_SCRIPT)
        self.assertIn('echo "Password superadmin (database): ********"', UBUNTU_SCRIPT)
        self.assertIn(
            'Retrieve superadmin password: sudo grep ^admin_passwd \\\"/etc/${OE_CONFIG}.conf\\\"',
            UBUNTU_SCRIPT,
        )
